  ...
  return (
    ...
    const fetchPromise = prevFetchPromise.then(async () => {
      ...
      if (selectVoice == "koeiromap") {
        ...
        buffer = await fetchAudioVoiceLocalTTS(screenplay.talk, localTTSServerUrl, localTTSModelId, localTTSBatchSize, localTTSSpeechRate).catch(
          () => null  
        );
      }
      lastTime = Date.now();
...
export const fetchAudioVoiceLocalTTS = async (
  talk: Talk,
  url: string,
  character: string,
  batchsize: number,
