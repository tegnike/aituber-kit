...
export function loadMixamoAnimation(url: string, vrm: VRM) {
  ...
  return loader.loadAsync(url).then((asset) => {
    ...
    clip.tracks.forEach((track) => {
      ...
      const mixamoRigName: MixamoVRMRigMapIndex =
        trackSplitted[0] as MixamoVRMRigMapIndex
      const vrmBoneName: VRMHumanBoneName = mixamoVRMRigMap[
        mixamoRigName
      ] as VRMHumanBoneName
      ...
      if (vrmNodeName != null) {
        ...
        if (track instanceof THREE.QuaternionKeyframeTrack) {
          ...
          for (let i = 0; i < track.values.length; i += 4) {
            ...
            _quatA.fromArray(flatQuaternion as unknown as THREE.QuaternionTuple)
