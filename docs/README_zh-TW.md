...
<div align="center">
   <h3>
      🚀 <a href="https://promotion.aituberkit.com/">推廣網站</a> 🚀
   </h3>
</div>
...
- **遊戲解說模式**：定期分析螢幕截圖，讓 AI 角色根據遊戲畫面自動生成並說出解說評論

### 5. 整合·擴展

- **外部整合模式**：透過 WebSocket 與伺服器應用程式連接，實現包含文字和圖像交換在內的更進階功能
- **幻燈片模式**：AI 角色自動展示幻燈片的模式
- **訊息接收功能**：透過專用 API 接受外部的文字和圖像，讓 AI 角色發言

## 支援的模型與服務

### 角色模型

- **3D 模型**：VRM 檔案（支援透過動作標籤控制姿勢和手勢）
...
初始設定完成後，只需雙擊啟動腳本即可啟動。

- **Windows**：雙擊 `LAUNCH.bat`
- **macOS**：雙擊 `LAUNCH.command`（如果沒有執行權限，請執行 `chmod +x LAUNCH.command`）

有關詳細配置和使用說明，請訪問[文檔網站](https://docs.aituberkit.com/zh/)。
...
1. 建立 [Vercel](https://vercel.com/) 帳戶並匯入 GitHub 倉庫。

2. 在 Vercel 儀表板中設定環境變數。新增所需的 API 金鑰（如 `OPENAI_API_KEY` 等）。可設定的環境變數請參考 `.env.example`。

3. 部署在推送到專案設定的 Production Branch 時自動執行（預設為 `main` 分支）。

### Cloudflare Workers

支援部署到 Cloudflare Workers。使用 [OpenNext](https://opennext.js.org/) 在 Cloudflare Workers 上執行 Next.js 應用程式。

1. 安裝 [Wrangler CLI](https://developers.cloudflare.com/workers/wrangler/) 並登入 Cloudflare 帳戶。

2. 根據需要修改 `wrangler.jsonc` 中的專案名稱。

3. 設定環境變數。
   - **前端設定（`NEXT_PUBLIC_*`）**：寫入 `.env` 檔案。建置時會嵌入到客戶端程式碼中。
   - **伺服器端 API 金鑰（`OPENAI_API_KEY` 等）**：設定為 Wrangler 密鑰。

```bash
cp .env.example .env
# 編輯 .env 設定 NEXT_PUBLIC_* 的值

# 將伺服器端 API 金鑰設定為 Wrangler 密鑰
npx wrangler secret put OPENAI_API_KEY
```

對每個所需的 API 金鑰重複執行 `wrangler secret put`。
...
- 建置時會自動設定 `NEXT_PUBLIC_RESTRICTED_MODE=true`，停用檔案系統 API。資產列表由建置時產生的清單提供。
- `public/` 下超過 25MB 的檔案或非 ASCII 檔案名的檔案將自動從部署中排除。
