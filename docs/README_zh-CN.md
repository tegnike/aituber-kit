...
<div align="center">
   <h3>
      🚀 <a href="https://promotion.aituberkit.com/">推广网站</a> 🚀
   </h3>
</div>
...
- **游戏解说模式**：定期分析屏幕截图，让AI角色根据游戏画面自动生成并说出解说评论

### 5. 集成·扩展

- **外部集成模式**：通过WebSocket与服务器应用程序连接，实现包含文本和图像交换在内的更高级功能
- **幻灯片模式**：AI角色自动展示幻灯片的模式
- **消息接收功能**：通过专用API接受外部的文本和图像，让AI角色发言

## 支持的模型和服务

### 角色模型

- **3D模型**：VRM文件（支持通过运动标签控制姿势和手势）
...
初始设置完成后，只需双击启动脚本即可启动。

- **Windows**：双击 `LAUNCH.bat`
- **macOS**：双击 `LAUNCH.command`（如果没有执行权限，请运行 `chmod +x LAUNCH.command`）
...
1. 创建[Vercel](https://vercel.com/)账户并导入GitHub仓库。

2. 在Vercel仪表板中设置环境变量。添加所需的API密钥（如`OPENAI_API_KEY`等）。可设置的环境变量请参考`.env.example`。

3. 部署在推送到项目设置的Production Branch时自动执行（默认为`main`分支）。

### Cloudflare Workers

支持部署到Cloudflare Workers。使用[OpenNext](https://opennext.js.org/)在Cloudflare Workers上运行Next.js应用程序。

1. 安装[Wrangler CLI](https://developers.cloudflare.com/workers/wrangler/)并登录Cloudflare账户。

2. 根据需要修改`wrangler.jsonc`中的项目名称。

3. 设置环境变量。
   - **前端设置（`NEXT_PUBLIC_*`）**：写入`.env`文件。构建时会嵌入到客户端代码中。
   - **服务器端API密钥（`OPENAI_API_KEY`等）**：设置为Wrangler密钥。

```bash
cp .env.example .env
# 编辑.env设置NEXT_PUBLIC_*的值

# 将服务器端API密钥设置为Wrangler密钥
npx wrangler secret put OPENAI_API_KEY
```

对每个所需的API密钥重复执行`wrangler secret put`。
...
- 构建时会自动设置`NEXT_PUBLIC_RESTRICTED_MODE=true`，禁用文件系统API。资产列表由构建时生成的清单提供。
- `public/`下超过25MB的文件或非ASCII文件名的文件将自动从部署中排除。
